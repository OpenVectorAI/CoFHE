...
Vector<CPUCryptoSystem::CipherText> inline homorphic_add(
    ...
    for (int i = 1; i < NUM_BITS; i++) {
        auto a_and_b = homorphic_and(client_node, ct1[i], ct2[i]);
        // auto a_and_b = homorphic_and(client_node, ct1[i], ct2[i]);
        // saves one extra mul
        ...
        auto a_xor_b_and_carry = homorphic_and(client_node, a_xor_b, carry);
        res.push_back(homorphic_xor(client_node, a_xor_b, carry));
        carry = homorphic_or(client_node, a_and_b, a_xor_b_and_carry);
    }
    return res;
}
...
Vector<CPUCryptoSystem::CipherText> inline homorphic_2s_complement(
    ...
    auto ones_complement = homorphic_not(client_node, ct);
    return homorphic_add(client_node, enc_one_bitwise, ones_complement);
}
...
Vector<CPUCryptoSystem::CipherText> inline homorphic_sub(
    ...
    const Vector<CPUCryptoSystem::CipherText>& ct1,
    const Vector<CPUCryptoSystem::CipherText>& ct2) {
    // return homorphic_add(client_node, ct1,
    // homorphic_2s_complement(client_node, ct2));
    Vector<CPUCryptoSystem::CipherText> res;
    auto neg_ct1 = homorphic_not(client_node, ct1);
    auto borrow = homorphic_and(client_node, ct2[0], neg_ct1[0]);
    res.push_back(homorphic_xor(client_node, ct1[0], ct2[0]));
    for (int i = 1; i < NUM_BITS; i++) {
        auto a_xor_b = homorphic_xor(client_node, ct1[i], ct2[i]);
        auto not_a_xor_b = homorphic_not(client_node, a_xor_b);
        auto b_in_and_not_a_xor_b =
            homorphic_and(client_node, borrow, not_a_xor_b);
        auto not_a_and_b = homorphic_and(client_node, ct2[i], neg_ct1[i]);
        res.push_back(homorphic_xor(client_node, a_xor_b, borrow));
        borrow = homorphic_or(client_node, b_in_and_not_a_xor_b, not_a_and_b);
    }
    return res;
}
...
CPUCryptoSystem::CipherText inline homorphic_eq(
    ...
    Vector<CPUCryptoSystem::CipherText>& ct1,
    Vector<CPUCryptoSystem::CipherText>& ct2) {
    auto sub = homorphic_sub(client_node, ct1, ct2);
    CPUCryptoSystem::CipherText sub_bits_sum = sub[0];
    for (int i = 1; i < NUM_BITS; i++) {
        sub_bits_sum = homorphic_or(client_node, sub_bits_sum, sub[i]);
    }
    return homorphic_not(client_node, sub_bits_sum);
}
...
CPUCryptoSystem::CipherText inline homorphic_gt(
    ...
    Vector<CPUCryptoSystem::CipherText>& ct1,
    Vector<CPUCryptoSystem::CipherText>& ct2) {
    return homorphic_not(client_node,
                         homorphic_or(client_node,
                                      homorphic_lt(client_node, ct1, ct2),
                                      homorphic_eq(client_node, ct1, ct2)));
}
