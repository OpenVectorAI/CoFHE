...
namespace CoFHE {
template <typename CryptoSystem> class SMPCClient {
  public:
    ...
    PlainText decrypt(CipherText ct) {
        ...
        // auto request =
        // PartialDecryptionRequest(PartialDecryptionRequest::DataType::SINGLE,
        ...
        for (size_t i = 0;
            ...
            pdrs.push_back(crypto_system_m.deserialize_part_decryption_result(
    ...
    Tensor<PlainText*> decrypt_tensor(Tensor<CipherText*> ct) {
        ...
        for (size_t i = 0;
             i < network_details_m.cryptosystem_details().threshold; i++) {
            pdrs.push_back(
                crypto_system_m.deserialize_part_decryption_result_tensor(
                    PartialDecryptionResponse::from_string((res[i])->data())
                        .data()));
        }
        auto res_ =
            crypto_system_m.combine_part_decryption_results_tensor(ct, pdrs);
        for (size_t i = 0;
             i < network_details_m.cryptosystem_details().threshold; i++) {
            delete res[i];
        }
  ...
  private:
    ...
    void init() {
        ...
        auto request = SetupNodeRequest(
            SetupNodeRequest::RequestType::BEAVERS_TRIPLET_REQUEST,
            BeaversTripletRequest(CACHE_SIZE).to_string());
        SetupNodeResponse* res;
