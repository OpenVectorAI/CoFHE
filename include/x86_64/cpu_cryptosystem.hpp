...
class CPUCryptoSystem {
  public:
    ...
    CPUCryptoSystem(uint32_t security_level, uint32_t k, bool compact = false)
        : rand_gen(BICYCL::RandGen(get_random_seed())),
          hsm2k(BICYCL::CL_HSM2k(security_level, k, rand_gen, compact)),
          sec_level(security_level), k(k) {
        init();
    }
    CPUCryptoSystem(uint32_t security_level, uint32_t k, std::string N,
                    bool compact = false)
        : rand_gen(BICYCL::RandGen(get_random_seed())),
          hsm2k(BICYCL::CL_HSM2k(BICYCL::Mpz(N), k, compact)),
          sec_level(security_level), k(k) {
        init();
    }
    CPUCryptoSystem(const CPUCryptoSystem& other)
        : rand_gen(BICYCL::RandGen(get_random_seed())), hsm2k(other.hsm2k),
          sec_level(other.sec_level), k(other.k) {
        init();
    }
    CPUCryptoSystem(CPUCryptoSystem&& other)
        : rand_gen(BICYCL::RandGen(get_random_seed())),
          hsm2k(std::move(other.hsm2k)), sec_level(other.sec_level),
          k(other.k) {
        init();
    }
    ...
    Vector<Vector<SecretKeyShare>> keygen(const SecretKey& sk, size_t threshold,
                                          size_t num_parties) const;
    CipherText encrypt(const PublicKey& pk, const PlainText& pt) const;
    Vector<CipherText*> encrypt_vector(const PublicKey& pk,
                                       const Vector<PlainText*>& pt) const;
    Tensor<CipherText*> encrypt_tensor(const PublicKey& pk,
                                       const Tensor<PlainText*>& pt) const;
    PlainText decrypt(const SecretKey& sk, const CipherText& ct) const;
    Vector<PlainText*> decrypt_vector(const SecretKey& sk,
                                      const Vector<CipherText*>& ct) const;
    Tensor<PlainText*> decrypt_tensor(const SecretKey& sk,
                                      const Tensor<CipherText*>& ct) const;
    PartialDecryptionResult part_decrypt(const SecretKeyShare& sks,
                                         const CipherText& ct) const;
    Vector<PartialDecryptionResult*>
    part_decrypt_vector(const SecretKeyShare& sks,
                        const Vector<CipherText*>& ct) const;
    Tensor<PartialDecryptionResult*>
    part_decrypt_tensor(const SecretKeyShare& sks,
                        const Tensor<CipherText*>& ct) const;
    PlainText combine_partial_decryption_results(
        const CipherText& ct,
        const Vector<PartialDecryptionResult>& pdrs) const;
    Vector<PlainText*> combine_partial_decryption_results_vector(
        const CipherText& ct,
        const Vector<PartialDecryptionResult*>& pdrs) const;
    ...
    CipherText add_ciphertexts(const PublicKey& pk, const CipherText& ct1,
                               const CipherText& ct2) const;
    CipherText scal_ciphertext(const PublicKey& pk, const PlainText& s,
                               const CipherText& ct) const;

    Vector<CipherText*>
    add_ciphertext_vectors(const PublicKey& pk, const Vector<CipherText*>& ct1,
                           const Vector<CipherText*>& ct2) const;
    Vector<CipherText*>
    scal_ciphertext_vector(const PublicKey& pk, const PlainText& s,
                           const Vector<CipherText*>& ct) const;
    Vector<CipherText*>
    scal_ciphertext_vector(const PublicKey& pk, const Vector<PlainText*>& s,
                           const Vector<CipherText*>& ct) const;

    Tensor<CipherText*>
    add_ciphertext_tensors(const PublicKey& pk, const Tensor<CipherText*>& ct1,
                           const Tensor<CipherText*>& ct2) const;
    Tensor<CipherText*>
    scal_ciphertext_tensors(const PublicKey& pk, const Tensor<PlainText*>& s,
                            const Tensor<CipherText*>& ct) const;

    PlainText generate_random_plaintext(const PlainText& min_bound,
                                        const PlainText& max_bound) const;
    PlainText add_plaintexts(const PlainText& pt1, const PlainText& pt2) const;
    PlainText multiply_plaintexts(const PlainText& pt1,
                                  const PlainText& pt2) const;
    Tensor<PlainText*>
    add_plaintext_tensors(const Tensor<PlainText*>& pt1,
                          const Tensor<PlainText*>& pt2) const;
    Tensor<PlainText*>
    multiply_plaintext_tensors(const Tensor<PlainText*>& pt1,
                               const Tensor<PlainText*>& pt2) const;
    PlainText negate_plaintext(const PlainText& s) const;
    Tensor<PlainText*>
    negate_plaintext_tensor(const Tensor<PlainText*>& pt) const;
    CipherText negate_ciphertext(const PublicKey& pk,
                                 const CipherText& ct) const;
    Tensor<CipherText*>
    negate_ciphertext_tensor(const PublicKey& pk,
                             const Tensor<CipherText*>& ct) const;
    ...
    String serialize_partial_decryption_result(
        const PartialDecryptionResult& pdr) const;
    ...
    String serialize_partial_decryption_result_tensor(
        const Tensor<PartialDecryptionResult*>& pdr_cpu) const;
    ...
    deserialize_partial_decryption_result(const String& data) const;
    ...
    Tensor<PartialDecryptionResult*>
    deserialize_partial_decryption_result_tensor(const String& data) const;
