...
class CoFHENodeResponse {
  public:
    ...
    class CoFHENodeResponseHeader {
      public:
        CoFHENodeResponseHeader(Status status, size_t data_size)
        ...
        static CoFHENodeResponseHeader from_string(std::string str) {
            ...
            return CoFHENodeResponseHeader(static_cast<Status>(status),
                                           data_size);
        }
    ...
    CoFHENodeResponse(CoFHENodeResponseHeader header, std::string data)
    ...
    CoFHENodeResponse(Status status, std::string data)
    ...
    static CoFHENodeResponse from_string(std::string str) {
        return CoFHENodeResponse(CoFHENodeResponseHeader::from_string(str),
                                 str.substr(str.find('\n') + 1));
    }

    static CoFHENodeResponse from_string(CoFHENodeResponseHeader header,
                                         std::string str) {
...
class CoFHENodeRequest {
  public:
    ...
    class CoFHENodeRequestHeader {
      public:
        CoFHENodeRequestHeader(RequestType type, size_t data_size)
        ...
        static CoFHENodeRequestHeader from_string(std::string str) {
            ...
            return CoFHENodeRequestHeader(static_cast<RequestType>(type),
                                          data_size);
        }
    ...
    CoFHENodeRequest(CoFHENodeRequestHeader header, std::string data)
        : header_m(header), data_m(data) {
        if (data.size() != header.data_size()) {
            throw std::runtime_error("Data size mismatch");
        }
    ...
    CoFHENodeRequest(RequestType type, std::string data)
    ...
    static CoFHENodeRequest from_string(std::string str) {
        return CoFHENodeRequest(CoFHENodeRequestHeader::from_string(str),
                                str.substr(str.find('\n') + 1));
    }

    static CoFHENodeRequest from_string(CoFHENodeRequestHeader header,
                                        std::string str) {
...
class CoFHENodeRequestHandler {
  ...
  private:
    ...
    handle_partial_decryption_request(const CoFHENodeRequest& request) {
        auto partial_decryption_request =
            PartialDecryptionRequest::from_string(request.data());
        auto partial_decryption_response =
            partial_decryption_handler_m.handle_request(
                partial_decryption_request);
        return CoFHENodeResponse(CoFHENodeResponse::Status::OK,
                                 partial_decryption_response.to_string());
    }
