...
class SetupNodeResponse {
  public:
    ...
    class SetupNodeResponseHeader {
      public:
        SetupNodeResponseHeader(Status status, size_t data_size)
            : status_m(status), data_size_m(data_size) {}
        ...
        static SetupNodeResponseHeader from_string(std::string str) {
            ...
            return SetupNodeResponseHeader(static_cast<Status>(status),
                                           data_size);
        }
    ...
    SetupNodeResponse(SetupNodeResponseHeader header, std::string data)
        : header_m(header), data_m(data) {
        if (data.size() != header.data_size()) {
            throw std::runtime_error("Data size mismatch");
        }
    }

    SetupNodeResponse(Status status, std::string data)
        : header_m(Status::OK, data.size()), data_m(data) {}
    ...
    std::string to_string() const { return header_m.to_string() + data_m; }

    static SetupNodeResponse from_string(std::string str) {
        return SetupNodeResponse(SetupNodeResponseHeader::from_string(str),
                                 str.substr(str.find('\n') + 1));
    }

    static SetupNodeResponse from_string(SetupNodeResponseHeader header,
                                         std::string str) {
...
class SetupNodeRequest {
  public:
    ...
    class SetupNodeRequestHeader {
      public:
        SetupNodeRequestHeader(RequestType type, size_t data_size)
        ...
        static SetupNodeRequestHeader from_string(std::string str) {
            ...
            return SetupNodeRequestHeader(static_cast<RequestType>(type),
                                          data_size);
        }
    ...
    SetupNodeRequest(SetupNodeRequestHeader header, std::string data)
        : header_m(header), data_m(data) {
        if (data.size() != header.data_size()) {
            throw std::runtime_error("Data size mismatch");
        }
    ...
    SetupNodeRequest(RequestType type, std::string data)
        : header_m(type, data.size()), data_m(data) {}
    ...
    static SetupNodeRequest from_string(std::string str) {
        return SetupNodeRequest(SetupNodeRequestHeader::from_string(str),
                                str.substr(str.find('\n') + 1));
    }

    static SetupNodeRequest from_string(SetupNodeRequestHeader header,
                                        std::string str) {
  ...
  private:
    ...
    SetupNodeResponse handle_join_as_node_request(const SetupNodeRequest& req) {
        JoinAsNodeRequest join_as_node_request =
            JoinAsNodeRequest::from_string(req.data());
        JoinAsNodeResponse join_as_node_response =
            join_as_node_handler_m.handle_request(join_as_node_request);
        return SetupNodeResponse(SetupNodeResponse::Status::OK,
                                 join_as_node_response.to_string());
    }

    SetupNodeResponse
    handle_network_details_request(const SetupNodeRequest& req) {
        NetworkDetailsRequest network_details_request =
            NetworkDetailsRequest::from_string(req.data());
        NetworkDetailsResponse network_details_response =
            network_details_handler_m.handle_request(
