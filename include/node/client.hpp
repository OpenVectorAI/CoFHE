...
class Client {
  public:
    Client(const std::string& address, const std::string& port,
           ...
           bool keep_session_alive = true)
        : io_context_m(), context_m(asio::ssl::context::sslv23),
          socket_m(io_context_m, context_m),
        ...
        context_m.load_verify_file(cert);
    ...
    void close() { end_session(); }

    bool is_session_alive() { return keep_session_alive_m; }
  ...
  private:
    ...
    void do_read(T** res) {
        asio::async_read_until(
            ...
            [this, res](const std::error_code& error,
                ...
                if (!error) {
                    try {
                        ...
                        if (read_buffer.size() > bytes_transferred) {
                            read_buffer =
                                read_buffer.erase(0, bytes_transferred);
                        } else {
                            read_buffer.clear();
                        }
                        auto res_header = Response::ResponseHeader::from_string(
                            res_header_str);
                        if (res_header.data_size() > read_buffer.size()) {
                            asio::async_read(
                                socket_m, asio::dynamic_buffer(read_buffer),
                                asio::transfer_exactly(res_header.data_size() -
                                                       read_buffer.size()),
                                [this, res_header,
                                 res](const std::error_code& error,
                                      size_t bytes_transferred) {
                                    if (!error) {
                                        process_request(res_header, read_buffer,
                                                        res);
                                    } else {
    ...
    template <typename T>
        requires ResponseType<T>
    void process_request(const Response::ResponseHeader& header,
                         std::string& data, T** res) {
        try {
            *res = new T(
                T::from_string(read_buffer.substr(0, header.data_size())));
...
inline auto make_client(const std::string& address, const std::string& port,
                        const std::string& cert = "./server.pem",
                        bool keep_session_alive = false) {
    return Client(address, port, cert, keep_session_alive);
}
