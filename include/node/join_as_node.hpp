...
class JoinAsNodeResponse {
  public:
    ...
    class JoinAsNodeResponseHeader {
      public:
        JoinAsNodeResponseHeader(Status status, ResponseType response_type,
                                 size_t data_size)
            : status_m(status), response_type_m(response_type),
              data_size_m(data_size) {}
        ...
        std::string to_string() const {
            return std::to_string(static_cast<int>(status_m)) + " " +
                   std::to_string(static_cast<int>(response_type_m)) + " " +
                   std::to_string(data_size_m) + "\n";
        }
        ...
        static JoinAsNodeResponseHeader from_string(std::string str) {
            ...
            return JoinAsNodeResponseHeader(
                static_cast<Status>(status),
                static_cast<ResponseType>(response_type), data_size);
        }
    ...
    // for cofhe node
    JoinAsNodeResponse(Status status, std::string public_key,
                       std::vector<std::string> secret_key_shares)
        : header_m(status, ResponseType::JOIN_AS_COFHE_NODE,
                   public_key.size() + 1),
          public_key_m(public_key), secret_key_shares_m(secret_key_shares) {
    ...
    // for compute node
    JoinAsNodeResponse(Status status)
        : header_m(status, ResponseType::JOIN_AS_COMPUTE_NODE, 0) {}

    // for error
    JoinAsNodeResponse(std::string error_message)
        : header_m(Status::ERROR, ResponseType::JOIN_AS_COFHE_NODE,
    ...
    const ResponseType& response_type() const {
        return header_m.response_type();
    }
    ...
    static JoinAsNodeResponse from_string(const std::string& str) {
        ...
        if (header.response_type() == ResponseType::JOIN_AS_COFHE_NODE) {
            ...
            return JoinAsNodeResponse(header.status(), public_key,
                                      secret_key_shares);
        } else if (header.response_type() ==
                   ResponseType::JOIN_AS_COMPUTE_NODE) {
    ...
    static JoinAsNodeResponse from_string(JoinAsNodeResponseHeader header,
...
class JoinAsNodeRequest {
  public:
    ...
    class JoinAsNodeRequestHeader {
      public:
        JoinAsNodeRequestHeader(RequestType type, size_t data_size)
            : type_m(type), data_size_m(data_size) {}
        ...
        static JoinAsNodeRequestHeader from_string(std::string str) {
            ...
            return JoinAsNodeRequestHeader(static_cast<RequestType>(type),
                                           data_size);
        }
    ...
    JoinAsNodeRequest(JoinAsNodeRequestHeader header, std::string ip,
                      std::string port)
        : header_m(header), ip_m(ip), port_m(port) {
        if (header_m.data_size() != ip_m.size() + 1 + port_m.size() + 1) {
            throw std::runtime_error("Data size mismatch");
        }
    }

    JoinAsNodeRequest(RequestType type, std::string ip, std::string port)
        : header_m(type, ip.size() + 1 + port.size() + 1), ip_m(ip),
          port_m(port) {}
    ...
    static JoinAsNodeRequest from_string(JoinAsNodeRequestHeader header,
                                         const std::string& str) {
        std::istringstream iss(str);
        std::string ip, port;
        iss >> ip >> port;
...
template <typename CryptoSystem> class JoinAsNodeRequestHandler {
  public:
    ...
    JoinAsNodeRequestHandler(const CryptoSystemDetails& cryptosystem_details,
        ...
        : cryptosystem_details_m(cryptosystem_details), self_node_m(self_node),
          ...
          total_nodes_m(cryptosystem_details.total_nodes) {
        init(reencryptor_details);
    }
    ...
    JoinAsNodeResponse handle_request(JoinAsNodeRequest req) {
        if (req.type() == JoinAsNodeRequest::RequestType::JOIN_AS_COFHE_NODE) {
            return handle_join_as_cofhe_node(req);
                   ...
                   JoinAsNodeRequest::RequestType::JOIN_AS_COMPUTE_NODE) {
  ...
  private:
    ...
    void init(const ReencryptorDetails& reencryptor_details) {
        ...
        for (const auto& party : sk_shares) {
            ...
            for (const auto& share : party) {
                secret_key_shares_m_party.push_back(
                    cryptosystem_m.serialize_secret_key_share(share));
            }
    ...
    JoinAsNodeResponse handle_join_as_cofhe_node(JoinAsNodeRequest req) {
        ...
        network_details_m.nodes().push_back(
            NodeDetails(req.ip(), req.port(), NodeType::CoFHE_NODE));
        auto res =
            JoinAsNodeResponse(JoinAsNodeResponse::Status::OK, public_key_m,
                               secret_key_shares_m[current_node_m]);
        current_node_m++;
        return res;
    }
    ...
    JoinAsNodeResponse handle_join_as_compute_node(JoinAsNodeRequest req) {
        std::lock_guard<std::mutex> lock(mtx_m);
        network_details_m.nodes().push_back(
            NodeDetails(req.ip(), req.port(), NodeType::COMPUTE_NODE));
