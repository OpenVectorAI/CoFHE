...
template <typename RequestHandlerImpl, typename RequestImpl,
          typename ResponseImpl>
class Session : public std::enable_shared_from_this<
                    Session<RequestHandlerImpl, RequestImpl, ResponseImpl>> {
  public:
    Session(asio::ssl::stream<tcp::socket> socket, RequestHandlerImpl& handler)
        : socket_m(std::move(socket)), handler_m(handler) {}
  ...
  private:
    using std::enable_shared_from_this<Session<RequestHandlerImpl, RequestImpl,
                                               ResponseImpl>>::shared_from_this;
    ...
    void do_handshake() {
        ...
        socket_m.async_handshake(asio::ssl::stream_base::server,
    ...
    void do_read() {
        ...
        asio::async_read_until(
            socket_m, asio::dynamic_buffer(read_buffer), '\n',
            [this, self](const std::error_code& error,
                         size_t bytes_transferred) {
                if (!error) {
                    try {
                        // the data can contain more data than the header
                        // specifies so in next call read the remaining data
                        auto header_str =
                            read_buffer.substr(0, bytes_transferred);
                        ...
                        auto header =
                            Request::RequestHeader::from_string(header_str);
                        if (header.data_size() > read_buffer.size()) {
                            asio::async_read(
                                ...
                                asio::transfer_exactly(header.data_size() -
                                                       read_buffer.size()),
                                [this, self,
                                 header](const std::error_code& error,
                                         size_t bytes_transferred) {
                                    if (!error) {
    ...
    void process_request(Request::RequestHeader header, std::string& data) {
        try {
            auto req = Request::from_string(header,
                                            data.substr(0, header.data_size()));
...
class Server {
  public:
    Server(const std::string& address, const std::string& port,
           ...
           const std::string& key = "./server_key.pem",
           size_t thread_pool_size = SERVER_THREAD_COUNT)
        : thread_pool_size_m(thread_pool_size), acceptor_m(io_context_m),
          context_m(asio::ssl::context::sslv23),
          request_handler_m(std::move(handler)),
          signals_m(io_context_m, SIGINT, SIGTERM) {
        signals_m.add(SIGINT);
        signals_m.add(SIGTERM);
        do_await_stop();
        context_m.set_options(asio::ssl::context::default_workarounds |
                              asio::ssl::context::no_sslv2 |
                              asio::ssl::context::single_dh_use);
        // context_m.set_verify_mode(asio::ssl::verify_peer);
        context_m.set_verify_mode(asio::ssl::verify_none);
        context_m.use_certificate_chain_file(cert);
        context_m.use_private_key_file(key, asio::ssl::context::pem);
  ...
  private:
    ...
    void do_accept() {
        acceptor_m.async_accept(
            [this](const std::error_code& error, asio::ip::tcp::socket socket) {
                if (!error) {
                    std::make_shared<
                        Session<RequestHandlerImpl, Request, Response>>(
                        boost::asio::ssl::stream<tcp::socket>(std::move(socket),
                                                              context_m),
                        request_handler_m)
                        ->start();
                }
    ...
    void do_await_stop() {
        signals_m.async_wait([this](boost::system::error_code e, int signal) {
            io_context_m.stop();
        });
    }
};
