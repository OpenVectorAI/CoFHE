...
template <typename T> class Storage {
  public:
    ...
    Storage(size_t n, value_type value = value_type())
        : shape(1, n), data(n, std::move(value)) {}
    Storage(size_t n, size_t m, value_type value = value_type())
        : shape(2, n), data(n * m, std::move(value)) {
        shape[1] = m;
    }
    Storage(Vector<size_t> shape, value_type value = value_type())
        : shape(std::move(shape)), data(1, std::move(value)) {
        int n = 1;
    ...
    Storage(Vector<size_t> shape, Vector<value_type> data) noexcept
        : shape(std::move(shape)), data(std::move(data)) {
        size_t num_elements = 1;
        for (size_t i = 0; i < this->shape.size(); i++) {
            num_elements *= this->shape[i];
        }
        if (num_elements != this->data.size() &&
            num_elements != this->data.capacity()) {
            throw std::invalid_argument(
                "Data size must be equal to product of shape");
        }
    }
    ...
    void push_back(value_type value) {
        return data.push_back(std::move(value));
    }
    ...
    iterator insert(size_t pos, const value_type& value) {
        data.insert(data.begin() + pos, value);
    }
    iterator insert(size_t pos, value_type&& value) {
        data.insert(data.begin() + pos, std::move(value));
    }
    iterator insert(size_t pos, const Vector<value_type>& values) {
        data.insert(data.begin() + pos, values.begin(), values.end());
    }
    iterator insert(size_t pos, Vector<value_type>&& values) {
        data.insert(data.begin() + pos, std::make_move_iterator(values.begin()),
                    std::make_move_iterator(values.end()));
    }
    iterator remove(size_t pos) { data.erase(data.begin() + pos); }
    iterator remove(size_t start, size_t end) {
        data.erase(data.begin() + start, data.begin() + end);
    }
    iterator remove(value_type* pos) { data.erase(pos); }
    iterator remove(value_type* start, value_type* end) {
    ...
    void resize(size_t n, value_type value = value_type()) {
        return data.resize(n, std::move(value));
    }
...
static inline void normalize_index(Vector<size_t>& index,
                                   const Vector<size_t>& shape,
                                   const Vector<size_t>& broadcast_degree) {
    for (size_t i = 0; i < index.size(); i++) {
        index[i] = index[i] % (shape[i] / broadcast_degree[i]);
    }
}

// check the stride(return value) after the function call to see if it is 0 or
// not to confirm if the stride was completed or not
static inline size_t
make_forward_stride(size_t stride, const Vector<size_t>& start,
                    const Vector<size_t>& end, size_t& current_section,
    ...
    while (stride > 0 && current_section < start.size()) {
        if (stride > end[current_section] - start[current_section] -
                         consumed_part_of_current_section) {
            stride -= end[current_section] - start[current_section] -
                      consumed_part_of_current_section;
...
static inline size_t
make_backward_stride(size_t stride, const Vector<size_t>& start,
                     const Vector<size_t>& end, size_t& current_section,
                     size_t& consumed_part_of_current_section) {
#ifdef DEBUG
    if (start.size() != end.size()) {
    ...
    if (current_section >= start.size()) {
        throw std::invalid_argument(
            "Current section must be less than start size");
    }
    if (consumed_part_of_current_section >
        end[current_section] - start[current_section]) {
        throw std::invalid_argument(
            "Consumed part of current section must be less than or equal "
            "capacity of current section");
    }
#endif
    if (consumed_part_of_current_section) {
        consumed_part_of_current_section -=
            min(consumed_part_of_current_section, stride);
        current_section--;
...
template <typename T, bool is_const> class Accessor;

template <typename T, bool is_const> class AccessorIterator {
  public:
    ...
    AccessorIterator(SharedPtr<Storage<base_value_type>> storage,
                     const Vector<size_t>& shape,
                     const Vector<size_t>& broadcast_degree,
                     const Vector<size_t>& start, const Vector<size_t>& end,
                     size_t stride) noexcept
        : storage_m(std::move(storage)), shape_m(shape),
          broadcast_degree_m(broadcast_degree), start_m(start), end_m(end),
          stride_m(stride), current_section(0),
          consumed_part_of_current_section(0) {
    ...
    const_value_type operator*() const noexcept {
        ...
        size_t consumed_part_of_current_section_local =
            consumed_part_of_current_section;
        auto res =
            make_forward_stride(stride_m, start_m, end_m, current_section_local,
                                consumed_part_of_current_section_local);
        if (res) {
            throw std::runtime_error("Stride not completed");
        }
        Vector<size_t> shape_local(shape_m.begin() + 1, shape_m.end());
        Vector<size_t> start_local(start_m.begin() + current_section,
                                   start_m.end());
        Vector<size_t> end_local(end_m.begin() + current_section, end_m.end());
        Vector<size_t> broadcast_degree_local(broadcast_degree_m.begin() + 1,
                                              broadcast_degree_m.end());
        start_local[0] =
            start_m[current_section] + consumed_part_of_current_section;
        end_local[current_section_local] =
            start_m[current_section_local] +
            consumed_part_of_current_section_local;
        return const_value_type(storage_m, shape_local, broadcast_degree_local,
        ...
        requires(!is_const)
    {
        ...
        end_local[current_section_local] =
            start_m[current_section_local] +
            consumed_part_of_current_section_local;
        return value_type(storage_m, shape_local, broadcast_degree_local,
                          start_local, end_local);
    }
    ...
    const_base_value_reference at(size_t n) const noexcept {
        ...
        auto stride =
            make_forward_stride(n * stride_m, start_m, end_m, local_section,
                                local_consumed_part_of_current_section);
        if (stride) {
            throw std::runtime_error("Stride not completed");
        }
        return storage_m->at(start_m[local_section] +
                             local_consumed_part_of_current_section);
    }
    ...
    AccessorIterator& operator++() noexcept {
        ...
        if (shape_m[0] - remaining_elements >=
            shape_m[0] / broadcast_degree_m[0]) {
            current_section = 0;
            consumed_part_of_current_section = 0;
        } else {
            size_t res =
                make_forward_stride(stride_m, start_m, end_m, current_section,
                                    consumed_part_of_current_section);
            if (res) {
                throw std::runtime_error("Stride not completed");
            }
    ...
    AccessorIterator& operator--() noexcept {
        ...
        size_t res =
            make_backward_stride(stride_m, start_m, end_m, current_section,
                                 consumed_part_of_current_section);
        if (res) {
            throw std::runtime_error("Stride not completed");
        }
    ...
    AccessorIterator& operator+=(size_t n) noexcept {
        ...
        size_t res =
            make_forward_stride(n * stride_m, start_m, end_m, current_section,
                                consumed_part_of_current_section);
        if (res) {
            throw std::runtime_error("Stride not completed");
        }
    ...
    AccessorIterator& operator-=(size_t n) noexcept {
        ...
        size_t res =
            make_backward_stride(n * stride_m, start_m, end_m, current_section,
                                 consumed_part_of_current_section);
        if (res) {
            throw std::runtime_error("Stride not completed");
        }
...
template <typename T, bool is_const = false> class Accessor {
  public:
    using base_value_type = T;
    using base_value_reference = base_value_type&;
    using base_value_pointer = base_value_type*;
    ...
    explicit Accessor(const Storage<base_value_type>& storage_m)
        : shape_m(storage_m.get_shape()), start_m(1, 0),
          end_m(1, storage_m.size()),
          storage_m(new Storage<base_value_type>(storage_m)) {
        one_init_broadcast_degree();
        calculate_stride();
    }
    explicit Accessor(const SharedPtr<Storage<base_value_type>>& storage_m)
        : shape_m(storage_m->get_shape()), start_m(1, 0),
          end_m(1, storage_m->size()), storage_m(storage_m) {
        one_init_broadcast_degree();
        calculate_stride();
    }
    Accessor(const SharedPtr<Storage<base_value_type>>& storage_m,
             const Vector<size_t>& shape_m, const Vector<size_t>& start_m,
             const Vector<size_t>& end_m)
        : shape_m(shape_m), start_m(start_m), end_m(end_m),
          storage_m(storage_m) {
        one_init_broadcast_degree();
        calculate_stride();
    }
    Accessor(SharedPtr<Storage<base_value_type>>&& storage_m,
             Vector<size_t>&& shape_m, Vector<size_t>&& start_m,
             Vector<size_t>&& end_m)
        : shape_m(std::move(shape_m)), start_m(std::move(start_m)),
          end_m(std::move(end_m)), storage_m(std::move(storage_m)) {
        one_init_broadcast_degree();
        calculate_stride();
    }
    Accessor(const Accessor<base_value_type>& other, Vector<size_t>&& shape_m,
             Vector<size_t>&& start_m, Vector<size_t>&& end_m)
        : shape_m(std::move(shape_m)), start_m(std::move(start_m)),
          end_m(std::move(end_m)), storage_m(other.storage_m) {
        one_init_broadcast_degree();
        calculate_stride();
    }
    Accessor(const SharedPtr<Storage<base_value_type>>& storage_m,
             const Vector<size_t>& shape_m,
             const Vector<size_t>& broadcast_defgree,
             const Vector<size_t>& start_m, const Vector<size_t>& end_m)
        : shape_m(shape_m), broadcast_degree_m(broadcast_defgree),
          start_m(start_m), end_m(std::move(end_m)), storage_m(storage_m) {
        calculate_stride();
    }
    ...
    Accessor<base_value_type, is_const> copy() const noexcept {
        ...
        SharedPtr<Storage<base_value_type>> new_storage(
            new Storage<base_value_type>(shape_m, new_storage_vec));
        return Accessor<base_value_type, is_const>(new_storage, shape_m, {0},
                                                   {new_storage->size()});
    }
    ...
    template <typename U>
        requires std::is_nothrow_copy_constructible_v<U> &&
                 IsNoThrowCallable<U, void, base_value_reference>
    ...
    template <typename U>
        requires std::is_nothrow_copy_constructible_v<U> &&
                 IsNoThrowCallable<U, void, const_base_value_reference,
                                   const Vector<size_t>&>
    ...
    // to be broadcastable either the shape of the tensor should be same or
    // divisible by current shape
    Accessor<T, is_const> broadcast(const Vector<size_t>& new_shape) noexcept {
        ...
        for (size_t i = 0; i < shape_m.size(); i++) {
            if (new_shape[i + size_diff] % shape_m[i]) {
                ...
                new_broadcast_degree[i + size_diff] = new_shape[i + size_diff] /
                                                      shape_m[i] *
                                                      broadcast_degree_m[i];
            }
        }
        for (size_t i = 0; i < size_diff; i++) {
            new_broadcast_degree[i] = new_shape[i];
        }
        return Accessor<T, is_const>(storage_m, new_shape, new_broadcast_degree,
                                     start_m, end_m);
    }
    ...
    void reshape(const Vector<size_t>& new_shape) {
        ...
        // we can reshape without copying if the new shape have same last
        // dimensions ??
        if (is_broadcasted()) {
            *this = copy();
        }
    ...
    const_base_value_reference at(size_t index) const noexcept {
        ...
        auto stride =
            make_forward_stride(index, start_m, end_m, current_section,
                                consumed_part_of_current_section);
        if (stride) {
            throw std::runtime_error("Stride not completed");
        }
        return storage_m->at(start_m[current_section] +
                             consumed_part_of_current_section);
    }

    const_base_value_reference at(size_t row_index,
        ...
        size_t current_section = 0;
        size_t consumed_part_of_current_section = 0;
        auto stride = make_forward_stride(
            row_index * strides_m[0] + column_index, start_m, end_m,
            current_section, consumed_part_of_current_section);
        if (stride) {
            throw std::runtime_error("Stride not completed");
        }
        return storage_m->at(start_m[current_section] +
    ...
    const_base_value_reference at(const Vector<size_t>& index) const noexcept {
        ...
        auto res = make_forward_stride(stride, start_m, end_m, current_section,
                                       consumed_part_of_current_section);
        if (res) {
            throw std::runtime_error("Stride not completed");
        }
    ...
    void set(const Vector<size_t>& start_dims,
             const Vector<base_value_type>& values) noexcept
        requires(!is_const)
    {
        size_t skip_ele = 0;
        ...
        for (size_t i = 0; i < values.size(); ++i) {
            ...
            if (already_used_part ==
                end_m[current_section] - start_m[current_section]) {
                already_used_part = 0;
                ++current_section;
            }
            *(storage_m->get_data() + start_m[current_section] +
              already_used_part) = values[i];
            ++already_used_part;
        }
    }
        ...
        requires(!is_const)
    {
        size_t current_section = 0;
        size_t consumed_part_of_current_section = 0;
        auto res = make_forward_stride(i, start_m, end_m, current_section,
                                       consumed_part_of_current_section);
        if (res) {
            throw std::runtime_error("Stride not completed");
        }
        storage_m->at(start_m[current_section] +
                      consumed_part_of_current_section) = value;
    }
        ...
        requires(!is_const)
    {
        ...
        size_t res_start_position = 0;
        auto stride_local =
            make_forward_stride(offset, start_m, end_m, current_section,
                                consumed_part_of_current_section);
        if (stride_local) {
            throw std::runtime_error("Stride not completed");
        }
        for (size_t i = 0; i < values.size(); i++) {
            ...
            storage_m->at(start_m[current_section] +
                          consumed_part_of_current_section) = values[i];
            ++consumed_part_of_current_section;
        }
    }
    ...
    iterator begin()
        requires(!is_const)
    {
        return iterator(storage_m, shape_m, broadcast_degree_m, start_m, end_m,
                        strides_m[0]);
    }
    const_iterator begin() const {
        return const_iterator(storage_m, shape_m, broadcast_degree_m, start_m,
                              end_m, strides_m[0]);
    }
    ...
    bool is_column_vector() const { return shape_m.size() == 1; }

    bool is_2d_matrix() const { return shape_m.size() == 2; }

    bool is_contiguous() const { return start_m.size() == 1; }
    ...
    void print(std::ostream& cout) {
        if (is_2d_matrix()) {
            ...
            auto print = [&](const T& val) noexcept { cout << val << " "; };
            walk(print);
            cout << std::endl;
        }
    }
  ...
  private:
    ...
    void calculate_stride() {
        ...
        for (int32_t i = strides_m.size() - 2; i >= 0; --i) {
            strides_m[i] =
                strides_m[i + 1] * (shape_m[i + 1] / broadcast_degree_m[i + 1]);
        }
    }
...
template <typename T> class Tensor {
  public:
    ...
    using value_type = typename Accessor<T>::value_type; // we need it to be T
                                                         // for 0 degree also
    ...
    using const_base_value_reference =
        typename Accessor<T>::const_base_value_reference;

    explicit Tensor(const_base_value_reference value)
        : accessor(SharedPtr<Storage<base_value_type>>(
              new Storage<base_value_type>(1, value))),
          is_zero_degree_m(true) {}
    Tensor(Vector<size_t> shape, base_value_type value = base_value_type())
        : accessor(SharedPtr<Storage<base_value_type>>(
              new Storage<base_value_type>(shape, value))) {}
    explicit Tensor(size_t n, base_value_type value = base_value_type())
        : accessor(SharedPtr<Storage<base_value_type>>(
              new Storage<base_value_type>(n, value))) {}
    Tensor(size_t n, size_t m, base_value_type value = base_value_type())
        : accessor(SharedPtr<Storage<base_value_type>>(
              new Storage<base_value_type>(n, m, value))) {}
    Tensor(const Vector<size_t>& shape, const Vector<base_value_type>& values)
        : accessor(SharedPtr<Storage<base_value_type>>(
              new Storage<base_value_type>(shape, values))) {}
    Tensor(Vector<size_t>&& shape, Vector<base_value_type>&& values)
        : accessor(
              SharedPtr<Storage<base_value_type>>(new Storage<base_value_type>(
                  std::move(shape), std::move(values)))) {}

    void set(Vector<size_t> start_dims, Vector<base_value_type> values) {
        accessor.set(start_dims, values);
    }
    void set(size_t i, base_value_type value) { accessor.set(i, value); }
    void set(Vector<size_t> dim, base_value_type value) {
        accessor.set(dim, value);
    }
    void set(size_t offset, Vector<base_value_type> values) {
        accessor.set(offset, values);
    }
    ...
    const_base_value_reference at(size_t index) const {
        return accessor.at(index);
    }
    base_value_reference at(const Vector<size_t>& index) {
        return accessor.at(index);
    }
    const_base_value_reference at(const Vector<size_t>& index) const {
        return accessor.at(index);
    }
    base_value_reference operator[](size_t index) { return accessor.at(index); }
    const_base_value_reference operator[](size_t index) const {
        return accessor.at(index);
    }
    base_value_reference at(size_t row_index, size_t column_index) {
        return accessor.at(row_index, column_index);
    }
    const_base_value_reference at(size_t row_index, size_t column_index) const {
        return accessor.at(row_index, column_index);
    }
    ...
    bool is_column_vector() const {
        if (is_zero_degree_m)
            return false;
        return accessor.is_column_vector();
    }
  ...
  private:
    ...
    Tensor(Accessor<base_value_type, false> accessor)
