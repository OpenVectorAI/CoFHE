...
int main(int argc, char* argv[]) {
    ...
    auto res_ct = cs.deserialize_ciphertext_tensor(res->data());

    // Step 7: Create compute request for reencryption
    ComputeRequest::ComputeOperationOperand decrypt_operand(
        ComputeRequest::DataType::TENSOR,
        ComputeRequest::DataEncrytionType::CIPHERTEXT, res->data());

    auto reencryptor = client_node.reencryptor();
    auto key_pair = reencryptor.generate_reencryption_key_pair();
    auto serialized_public_key =
        reencryptor.serialize_reencryption_public_key(key_pair);
    ComputeRequest::ComputeOperationOperand pub_key_op(
        ComputeRequest::DataType::SINGLE,
        ComputeRequest::DataEncrytionType::PLAINTEXT, serialized_public_key);

    ComputeRequest::ComputeOperationInstance decrypt_operation(
        ComputeRequest::ComputeOperationType::BINARY,
        ComputeRequest::ComputeOperation::REENCRYPT,
        {decrypt_operand, pub_key_op});
    ...
    auto reencrypted_result = ClientNode<CPUCryptoSystem>::ReencryptorType::
        split_reencrypted_messages(res->data());
    auto result_tensor =
        reencryptor.decrypt_tensor(reencrypted_result, res_ct, key_pair);
    result_tensor.flatten();

    std::cout << "Resulting matrix after multiplication and decryption:"
              << std::endl;
    ...
    auto duration_d =
        std::chrono::duration_cast<std::chrono::microseconds>(stop_d - stop);

    std::cout << "Time taken by multiplication: " << duration.count()
              << " microseconds" << std::endl;
    std::cout << "Time taken by decryption: " << duration_d.count()
              << " microseconds" << std::endl;
